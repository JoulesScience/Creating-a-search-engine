    ...
    for i in range(0, numloops):
        ...
        for page in graph:
            ...
            for value in graph.values():
                if page in value:
                    suma += d*ranks[graph.keys()[graph.values().index(value)]] / len(graph[graph.keys()[graph.values().index(value)]])                  
...
for page in graph:
    for i in graph.values():
        if page in i:
            inlinks.append(graph.keys()[graph.values().index(i)])
    break        
            

##Propuesto por Udacity
##def compute_ranks(graph):
